...
const CIRC = 2 * Math.PI * 52; // circumference of r=52 circle in SVG
...
function makeTile(item, {eager=false} = {}){
  ...
  if (eager) { img.src = imageUrlAt(item, 0, 400); } else { lazyThumb(img, imageUrlAt(item, 0, 400)); }
...
async function loadNextPage({nocache=false} = {}) {
  ...
  try {
    ...
    // Build tiles; on the first page, eagerly set src so loader can be truly accurate
    const eager = nextPage === 1;
    const imgsToTrack = [];
    ...
    // If first page: track actual image load progress
    if (eager && imgsToTrack.length) {
    ...
    // Fail-safe: remove loader if something goes wrong on first page
    if (!firstPageLoaded) hideLoader();
...
function trackFirstPageLoads(imgEls){
  return new Promise((resolve)=>{
    ...
    // Edge case: if an image is already complete from cache
    imgEls.forEach(img=>{
      if (img.complete && img.naturalWidth > 0) {
        update();
      } else {
        img.addEventListener('load', update, { once:true });
        img.addEventListener('error', update, { once:true }); // count errors so loader won't hang
      }
    });

    // Start at a tiny non-zero to show motion
